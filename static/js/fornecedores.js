...
let currentProdutosFornecedorPage = 1;

const userLevelFornecedores = (
  window.currentUserInfo?.level || ""
).toLowerCase();
const canManageFornecedor = ["admin", "gerente"].includes(
  userLevelFornecedores,
);
...
document.addEventListener("DOMContentLoaded", () => {
  currentFiltersFornecedores.ativo =
    !document.getElementById("mostrarInativos")?.checked;
  carregarFornecedores();

  configurarEventos();
  configurarBusca();
  configurarFiltrosFornecedores();
});
...
function configurarEventos() {
  ...
  const btnAtualizarFornecedor = document.getElementById(
  ...
  if (modalProdutos) {
    modalProdutos.addEventListener("hidden.bs.modal", () => {
      ...
      document.getElementById("produtos-fornecedor-tabela").innerHTML =
...
function configurarFiltrosFornecedores() {
  ...
  if (chkMostrarInativos) {
    chkMostrarInativos.addEventListener("change", (e) => {
      currentFiltersFornecedores.ativo = !e.target.checked;
      currentPageFornecedores = 1;
      carregarFornecedores();
    });
  }
...
function carregarFornecedores() {
  ...
  const params = {
    ...
    ativo:
      currentFiltersFornecedores.ativo === null
        ? null
        : String(currentFiltersFornecedores.ativo),
  };
  ...
  axios
    .get(API_ROUTES.FORNECEDORES_LISTAR, { params })
    .then((response) => {
      const data = response.data;
      fornecedoresList = data.fornecedores || [];
      totalPagesFornecedores = data.pages || 1;
      currentPageFornecedores = data.page || 1;
      renderizarTabelaFornecedores(fornecedoresList);
      renderizarPaginacaoFornecedores();
    })
    .catch((error) => {
      console.error("Erro ao carregar fornecedores:", error);
      showNotification("Erro ao carregar fornecedores", "danger");
      document.getElementById("fornecedores-tabela").innerHTML =
        `<tr><td colspan="7" class="text-center text-danger">Erro ao carregar. Tente novamente.</td></tr>`;
    })
    .finally(() => {
      toggleLoading(false);
    });
...
function renderizarTabelaFornecedores(listaFornecedores) {
  ...
  listaFornecedores.forEach((fornecedor) => {
    ...
    const toggleStatusIcon = fornecedor.ativo
      ? "fa-toggle-on text-success"
      : "fa-toggle-off text-secondary";
    const toggleStatusTitle = fornecedor.ativo ? "Desativar" : "Ativar";
    const nomeFornecedorEscapado = String(fornecedor.nome || "").replace(
      /'/g,
      "\\'",
    );
    ...
    const actionDivider =
      actionToggle || actionEdit || actionDanger
...
function adicionarFornecedor() {
  if (
    !assertFornecedorPermission(
      canManageFornecedor,
      "Apenas admin e gerente podem adicionar fornecedores.",
    )
  )
    return;
  ...
  const observacoes = document
    .getElementById("observacoesFornecedor")
    .value.trim();
  ...
  axios
    .post(API_ROUTES.FORNECEDORES_LISTAR, {
      // Correção aqui
      nome,
      cnpj: cnpj || null,
    ...
    .then((response) => {
      ...
      const modal = bootstrap.Modal.getInstance(
        document.getElementById("modalAdicionarFornecedor"),
      );
      modal.hide();
    ...
    .catch((error) => {
      ...
      const mensagem =
        error.response?.data?.error || "Erro ao adicionar fornecedor";
...
function editarFornecedor(id) {
  if (
    !assertFornecedorPermission(
      canManageFornecedor,
      "Apenas admin e gerente podem editar fornecedores.",
    )
  )
    return;
  ...
  axios
    .get(API_ROUTES.FORNECEDOR_DETALHES(id))
    .then((response) => {
      ...
      document.getElementById("editNomeFornecedor").value =
        fornecedor.nome || "";
      document.getElementById("editCnpjFornecedor").value =
        fornecedor.cnpj || "";
      document.getElementById("editTelefoneFornecedor").value =
        fornecedor.telefone || "";
      document.getElementById("editEmailFornecedor").value =
        fornecedor.email || "";
      document.getElementById("editEnderecoFornecedor").value =
        fornecedor.endereco || "";
      document.getElementById("editContatoFornecedor").value =
        fornecedor.contato || "";
      document.getElementById("editObservacoesFornecedor").value =
...
function atualizarFornecedor() {
  if (
    !assertFornecedorPermission(
      canManageFornecedor,
      "Apenas admin e gerente podem atualizar fornecedores.",
    )
  )
    return;
  ...
  axios
    .put(API_ROUTES.FORNECEDOR_DETALHES(id), {
      nome,
      cnpj: cnpj || null,
      telefone: telefone || null,
      email: email || null,
    ...
    .catch((error) => {
      ...
      const mensagem =
        error.response?.data?.error || "Erro ao atualizar fornecedor";
      showNotification(mensagem, "danger");
    })
    .finally(() => {
      toggleLoading(false);
...
function excluirFornecedor(id, nome) {
  if (
    !assertFornecedorPermission(
      canDeleteFornecedor,
      "Apenas admin pode excluir fornecedores.",
    )
  )
    return;
...
function confirmarExclusaoFornecedor() {
  ...
  const id = document.getElementById("btnConfirmarExclusao").dataset
    .fornecedorId; // Pega do botão
  if (!id) return;

  toggleLoading(true);

  axios
    .delete(API_ROUTES.FORNECEDOR_DETALHES(id))
    .then((response) => {
    ...
    .catch((error) => {
      ...
      const mensagem =
        error.response?.data?.error || "Erro ao excluir fornecedor";
      showNotification(mensagem, "danger");
    ...
    .finally(() => {
      toggleLoading(false);
      delete document.getElementById("btnConfirmarExclusao").dataset
        .fornecedorId; // Limpa o ID
    });
}
...
function alternarStatusFornecedor(id) {
  if (
    !assertFornecedorPermission(
      canManageFornecedor,
      "Apenas admin e gerente podem alterar status de fornecedores.",
    )
  )
    return;
  ...
  axios
    .post(API_ROUTES.FORNECEDOR_ALTERNAR_STATUS(id))
    .then((response) => {
      showNotification(
        `Fornecedor ${response.data.ativo ? "ativado" : "desativado"} com sucesso!`,
        "success",
      );
...
function verProdutosFornecedor(id, nome, page = 1) {
  ...
  const paginacaoContainer = document.getElementById(
    "paginacao-produtos-fornecedor",
  );

  if (modalNome) modalNome.textContent = nome;
  if (tabelaProdutos)
    tabelaProdutos.innerHTML = `<tr><td colspan="5" class="text-center">Carregando produtos...</td></tr>`; // Colspan 5
  ...
  axios
    .get(API_ROUTES.FORNECEDOR_PRODUTOS(id), {
      params: {
    ...
    .then((response) => {
      ...
      if (tabelaProdutos) {
        ...
        if (produtos.length === 0) {
          ...
          produtos.forEach((produto) => {
            const precoCompra = produto.preco_compra
              ? `R$ ${produto.preco_compra.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
              : "-";
      ...
      if (page === 1) {
        // Só abre na primeira chamada
        const modal = new bootstrap.Modal(
          document.getElementById("modalProdutosFornecedor"),
        );
        modal.show();
      }
    })
    .catch((error) => {
      ...
      if (tabelaProdutos)
        tabelaProdutos.innerHTML = `<tr><td colspan="5" class="text-center text-danger">Erro ao carregar produtos.</td></tr>`;
    })
    .finally(() => {
      toggleLoading(false);
    });
